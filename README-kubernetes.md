...
```bash
git clone https://github.com/darrellschrag/get-names
cd get-names
```
...
2. Identify your **Container Registry** by running `ibmcloud cr info` (Ex: us.icr.io)
...
3. Build and tag (`-t`)the docker image by running the command below replacing REGISTRY and NAMESPACE with he appropriate values.
   ...
   ```
   Example: `docker build . -t us.icr.io/mynamespace/myapp:v1.0.0
...
- **Paid Cluster**: Expose the service using an External IP and Loadbalancer
  ```
  kubectl expose deployment get-names --type LoadBalancer --port 8080 --target-port 8080
  ```
- **Paid Cluster**: Or expose the service using the service.yaml file
  ```
  kubectl create -f kubernetes/service.yaml
  ```
- **Free Cluster**: Use the Worker IP and NodePort
  ```bash
  kubectl expose deployment get-names --type NodePort --port 8080 --target-port 8080
  ```
...
```shell
kubectl get pods -l app=get-names
```

**Standard (Paid) Cluster:**

1. Identify your LoadBalancer Ingress IP using `kubectl get service get-names`
...
2. Identify the Node Port using `kubectl describe service get-names`
3. Access your application at `http://<WORKER-PUBLIC-IP>:<NODE-PORT>/`


## Clean Up
```bash
kubectl delete deployment,service -l app=get-names
